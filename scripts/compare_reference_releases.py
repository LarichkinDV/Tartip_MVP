...
def compare_values(
    old: Any, new: Any, field_path: str = "payload"
) -> list[dict[str, Any]]:
    if old == new:
        return []
...
def change_detail(
    field_path: str, change_type: str, old: Any, new: Any
) -> dict[str, Any]:
    return {
...
def compare_releases(
    old_path: Path, new_path: Path, dependencies_path: Path | None = None
) -> dict[str, Any]:
    old_records = records_by_natural_key(load_data(old_path))
    new_records = records_by_natural_key(load_data(new_path))

    old_hashes = {
        key: canonical_payload_hash(record_payload(record))
    ...
    changed_details = {
        key: compare_values(
            record_payload(old_records[key]), record_payload(new_records[key])
        )
        for key in changed
    }
    ...
    for dependency in load_dependencies(dependencies_path):
        ...
        if natural_key in changed_or_deleted and dependency.get(
            "required_review_on_change", True
        ):
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compare two small reference release fixtures."
    )
    ...
    parser.add_argument(
        "--dependencies", type=Path, help="Optional rule dependency fixture path."
    )
    parser.add_argument(
        "--fixture", action="store_true", help="Use bundled fake fixtures."
    )
    ...
    print(
        json.dumps(
            compare_releases(old_path, new_path, dependencies_path),
            indent=2,
            ensure_ascii=False,
        )
    )
